...
public sealed class AuthService : IAuthService
{
    ...
    public async Task<byte[]> ComputeHashAsync(string input, CancellationToken cancellationToken = default)
    {
        return await ComputeHashAsync(Encoding.ASCII.GetBytes(input), cancellationToken);
    }

    public async Task<byte[]> ComputeHashAsync(byte[] input, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<(byte[] SaltHash, byte[] FinalizedOutput)> EncryptInputAsync(string input, CancellationToken cancellationToken = default)
    {
    ...
    public string GenerateRandomCryptoSafeString()
    {
